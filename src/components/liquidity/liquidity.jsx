...
class Liquidity extends Component {
  constructor(props) {
    ...
    const preSelectedPoolMatches =
      window.location.hash.match(/pool=([a-z0-9/-]+)/i);
    const preSelectedPool =
      preSelectedPoolMatches === null ? null : preSelectedPoolMatches[1];
    ...
    if (pools && pools.length > 0) {
      ...
      if (v2PoolsArr.length > 0) {
        selectedPool =
          !v2PoolsArr || v2PoolsArr.length === 0
            ? null
            : preSelectedPool !== null
            ? v2PoolsArr.find(({ id }) => id === preSelectedPool)
            : v2PoolsArr[0];
      }
    }
