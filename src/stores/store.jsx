...
class Store {
  ...
  _getPoolData = async (web3, pool, account, callback) => {
    try {
      ...
      if (pool.version === 1) {
        curveFactoryContract = new web3.eth.Contract(
          config.curveFactoryABI,
          config.curveFactoryAddress
        );
      } else {
      ...
      const poolBalances = await curveFactoryContract.methods
        .get_balances(pool.address)
        .call();
      const isPoolSeeded = sumArray(poolBalances) !== 0;

      let coins = await curveFactoryContract.methods
        .get_underlying_coins(pool.address)
        .call();
